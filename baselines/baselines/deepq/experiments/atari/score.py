...
def play(env, acts, stochastic, video_path, game_name, attack=None, q_func=None):
    ...
    #attack_rates = np.arange(0, 1.1, 0.1)
    attack_rates = [1.0]
    index_rate = 0
    ep_cnt = 0
    ep_per_round = 3
    ...
    while True:
        ...
        if attack == 'carliniL2':
            ...
            is_attack = True if np.random.rand() < attack_rates[index_rate] else False
            if is_attack:
                attack_count += 1
            ...
            if step >= 4:
                ...
                if len(pred_obs) == 4:
                    ...
                    for threshold in thresholds:
                        ...
                        if np.argmax(sq_final_q_values) != final_act:
                            #final_act = np.argmax(sq_final_q_values)
                            #final_act = env.action_space.sample()
                            if is_attack:
                                tp_sq[threshold] += 1
                            else:
                                fp_sq[threshold] += 1
        ...
        if len(info["rewards"]) > num_episodes:
            ...
            print("Attack rate: ", attack_rates[index_rate])

            print(info["rewards"][-1])
            num_episodes = len(info["rewards"])

            ep_cnt += 1
            if ep_cnt == ep_per_round:
                ep_cnt = 0
                index_rate += 1
                if index_rate == len(attack_rates):
                    exit()
