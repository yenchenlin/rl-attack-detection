...
def test_gen(sess, obs, act, gt, mean, model, n_actions, step):
    ...
    #error = np.sum(np..square(gt[:, :, -1][:, :, np.newaxis] - pred[0, :, :, :]))
...
def play(env, acts, stochastic, video_path, game_name, attack=None, q_func=None):
    ...
    while True:
        ...
        if attack == 'carliniL2':
            ...
            is_attack = True if np.random.rand() < 1.0 else False
            if is_attack:
                attack_count += 1
            ...
            if step >= 4:
                ...
                pred_obs.append(test_gen(sess, old_obs + np.random.randn(84, 84, 4) * 0.03, old_action, np.array(obs), mean, model,
                    env.action_space.n, step))
                adv_pred_obs.append(test_gen(sess, adv_env(np.array(old_obs)[None], stochastic=stochastic)[0] * 255.0, old_action, np.array(obs), mean, model,
                    env.action_space.n, step))

                rec_loss += np.sum(np.square(np.array(obs)[:, :, -1] - pred_obs[-1]))
                adv_rec_loss += np.sum(np.square(np.array(obs)[:, :, -1] - adv_pred_obs[-1]))
        ...
        if len(info["rewards"]) > num_episodes:
            ...
            if attack != None:
                ...
                print("Reconstruction Loss: ", rec_loss/step)
                print("Adversarial Reconstruction Loss: ", adv_rec_loss/step)
