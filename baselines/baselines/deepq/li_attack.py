...
#INITIAL_CONST = 5e-1    # the first value of c to start at
INITIAL_CONST = 1e+5    # the first value of c to start at

LEARNING_RATE = 5e-3    # larger values converge faster to less accurate results

#LARGEST_CONST = 1e-5    # the largest value of c to go up to before giving up
#LARGEST_CONST = 2e+1    # the largest value of c to go up to before giving up
LARGEST_CONST = 1e+5 + 1    # the largest value of c to go up to before giving up
...
class CarliniLi:
    ...
    def gradient_descent(self, sess, model):
        def compare(x,y):
            if self.TARGETED:
                ...
                return x != y
        shape = (1,self.image_size,self.image_size,self.num_channels)
        ...
        tlab = tf.placeholder(tf.float32, (1,self.num_labels))
        ...
        with tf.variable_scope('deepq', reuse=True):
            ...
            output = model(newimg, self.num_labels, scope="q_func", reuse=True)
            #orig_output = model.predict(tf.tanh(timg)/2)
            orig_output = model(tf.tanh(timg)/2, self.num_labels, scope="q_func", reuse=True)
    ...
    def attack_single(self, img, target):
        ...
        # the previous image
        prev = np.copy(img).reshape((1,self.image_size,self.image_size,self.num_channels))
        tau = 1.0
